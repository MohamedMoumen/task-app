...
export const JiraCard: FC<CustomCard> = ({ title, storyPoints, id, ticketType, assigneeId, prLink, description, assignee, priority }) => {
            ...
            {prLink ? (
              <Avatar sx={{ cursor: 'pointer', width: 24, height: 24, bgcolor: 'transparent', mr: '5px' }}>
                <PrLink href={prLink} target='_blank' rel='noopener noreferrer'>
                  <PrIcon src={PrIconSrc.src} alt='' />
                </PrLink>
              </Avatar>
            ) : null}
            <Chip sx={{ mr: '10px' }} size='small' label={assignee?.name || 'N/A'} />
            <Avatar sx={{ width: 24, height: 24 }} src={`https://mui.com/static/images/avatar/${assigneeId}.jpg`} />
          </Stack>
        </Box>
          <Stack direction='column'>
            Priority: <Typography variant='caption'>{priority}</Typography>
          </Stack>
      </CardContent>
    </Card>
  )
